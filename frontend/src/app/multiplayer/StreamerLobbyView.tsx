      ...
      <SurfaceCard className="flex flex-col gap-4">
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-2">
            ...
            <h2 className="text-3xl font-semibold leading-tight text-white">Lance le flux interactif</h2>
            <p className="text-sm text-white/70">Le chat répond via code, tu contrôles le rythme.</p>
          </div>
        ...
        <p className="text-xs text-white/60">Projette ensuite le code, le chat rejoindra depuis ses téléphones.</p>
      </SurfaceCard>

      <SurfaceCard className="space-y-3">
        <h3 className="text-lg font-semibold text-white">Rejoindre le flux</h3>
        <p className="text-sm text-white/70">Colle le code projeté, connecte Spotify, tu entres directement.</p>
    ...
    <SurfaceCard className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-col items-end gap-2 text-right text-xs uppercase tracking-[0.3em] text-white/60">
          {props.hostUser ? (
            <div className="rounded-xl border border-white/15 bg-[#0f0f0f] px-3 py-2 text-right text-[11px] text-white/70">
              <p className="uppercase tracking-[0.3em] text-white/50">Hôte</p>
              <p className="text-sm font-semibold text-white">{props.hostUser.username || `#${props.hostUser.user_id}`}</p>
            </div>
          ) : null}
          <div>
            <div>Code</div>
      ...
      <div className="grid gap-4 md:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
        <SurfaceCard className="space-y-3 border-white/10 bg-[#0f0f0f]">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs uppercase tracking-[0.3em] text-white/60">{audience.length} présent(s)</span>
          </div>
          <ParticipantPanel
        ...
        <SurfaceCard className="space-y-3 border-white/10 bg-[#0f0f0f]">
          ...
          <h4 className="text-xl font-semibold text-white">Un seul bouton : lancer</h4>
          <p className="text-sm text-white/70">Quand tout le monde est prêt, clique sur “Lancer” depuis la vue jeu.</p>
          <div className="rounded-xl border border-white/10 bg-[#0a0a0a] px-4 py-3 text-sm text-white/70">
            <p className="text-xs uppercase tracking-[0.3em] text-white/50">Code à projeter</p>
            <p className="mt-1 text-3xl font-semibold tracking-[0.24em] text-white" style={{ color: accent }}>
              {roomCode}
            </p>
            <p className="mt-2 text-xs text-white/55">Les joueurs collent ce code, connectent Spotify et entrent directement.</p>
          </div>
        </SurfaceCard>
      </div>
    </SurfaceCard>
